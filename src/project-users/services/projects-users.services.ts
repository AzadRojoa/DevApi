...
export class ProjectsUserServices {
  ...
  async createProjectUser(
    ...
    Projectbody: ProjectUserDTO,
  ): Promise<ProjectUser> {
    if (user.role === 'Employee') {
      throw new UnauthorizedException();
    }

    const presentProj = await this.projetService.findOnebyid(
    ...
    if (present.length !== 0) {
      ...
      return this.projectUserRepository.save(
        this.projectUserRepository.create(Projectbody),
      );
    }
  }

  findOnebyProjectIdAndUserid(
    id: string,
    userid: string,
  ): Promise<ProjectUser> {
  ...
  findOnebyid(id: string, user: PasswordLessUser): Promise<ProjectUser> {
    if (user.role === 'Employee') {
  ...
  findall(user: PasswordLessUser): Promise<ProjectUser[]> {
    if (user.role === 'Employee') {
  ...
  getAll(options?: FindManyOptions<ProjectUser>): Promise<ProjectUser[]> {
